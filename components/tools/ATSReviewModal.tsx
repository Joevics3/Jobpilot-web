            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600">View your previous ATS CV reviews</p>
                </div>
                <button
              ...
              {sessionHistory.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">No review sessions yet</p>
                  <button
                    onClick={() => setStep('cv-selection')}
                ...
                <div className="space-y-3">
                  {sessionHistory.map((histSession) => {
                    const date = new Date(histSession.timestamp);
                    ...
                    return (
                      <div
                        ...
                        onClick={() => loadSessionById(histSession.id)}
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              <h4 className="font-semibold text-gray-900">{histSession.cvName}</h4>
                              <span className="px-2 py-1 rounded text-xs font-medium bg-green-100 text-green-700">
                                {histSession.reviewType === 'cv-job' ? 'Job-Specific' : 'General'}
                              </span>
                            </div>
                            {histSession.jobTitle && (
                              <p className="text-sm text-gray-600 mb-2">
                                {histSession.jobTitle} {histSession.jobCompany && `• ${histSession.jobCompany}`}
                              </p>
                            )}
                            <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
                              ...
                              <div className="flex items-center gap-2">
                                <span className="font-medium" style={{ color: getScoreColor(histSession.overallScore) }}>
                                  Score: {histSession.overallScore}/100
                                </span>
                              </div>
                            </div>
                            {histSession.lastMessage && (
                              <p className="text-sm text-gray-600 line-clamp-2">{histSession.lastMessage}</p>
                            )}
                          </div>
                          <ArrowRight size={20} className="text-gray-400 ml-4 flex-shrink-0" />
                        </div>
                      </div>
                    );
                  })}
