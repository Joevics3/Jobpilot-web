...
export default function OptimizedTownPage() {
  ...
  }, [formattedState, formattedTown]);

  const fetchJobsByTown = async () => {
    try {
      setLoading(true);
      
      // ONE fetch: Get latest 200 active jobs
      const { data, error } = await supabase
        .from('jobs')
        .select('id, title, company, location, salary_range, type, remote, posted_date, created_at')
        .eq('status', 'active')
      ...
      // Simple JavaScript filtering - filter for state AND town
      const townFilteredJobs = (data || []).filter((job) => {
        // First check state
        let matchesState = false;
        if (typeof job.location === 'string') {
          matchesState = job.location.toLowerCase().includes(formattedState.toLowerCase());
        } else if (job.location && typeof job.location === 'object') {
          const locState = job.location.state || '';
          matchesState = locState.toLowerCase() === formattedState.toLowerCase();
        }

        if (!matchesState) return false;

        // Then check town/city
        if (typeof job.location === 'string') {
      ...
      // Take first 50 jobs
      const limitedJobs = townFilteredJobs.slice(0, 50);
  ...
  const transformJobToCard = (job: any) => {
      ...
      <div className="text-white py-8 px-6" style={{ backgroundColor: '#2563EB' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-lg text-white">
            ...
            {totalJobs > 50 && ` (showing latest 50)`}
          </p>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {jobs.map((job) => (
              <JobCard
                key={job.id}
                job={transformJobToCard(job)}
                savedJobs={savedJobs}
                appliedJobs={appliedJobs}
                onSave={handleSave}
                onApply={handleApply}
                onShowBreakdown={() => {}} // No matching breakdowns
              />
            ))}
          </div>
        )}
        ...
        {totalJobs > 50 && (
          <div className="mt-8 text-center">
            <p className="text-gray-600 mb-4">
              Showing 50 of {totalJobs} jobs in {formattedTown}
            </p>
            <p className="text-sm text-gray-500">
              For more jobs, try refining your search or check back later for new postings.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="bg-blue-600 rounded-lg p-8 text-white text-center">
          ...
          <p className="text-blue-100 mb-6 max-w-2xl mx-auto">
            Explore more opportunities in other towns within {formattedState} or across Nigeria.
          </p>
