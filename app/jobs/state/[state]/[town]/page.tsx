...
export default function JobsByTownPage() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      setOtherStateJobs(otherJobs.map(transformJobToUI));
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-3">
              <h2 className="text-xl font-bold text-gray-900">Jobs in {formattedTown}</h2>
              {jobs.map((job) => (
                <JobCard
                  key={job.id}
                  job={job}
            ...
            {/* Other Jobs in State */}
            {otherStateJobs.length > 0 && (
              <div className="space-y-3">
                <h2 className="text-xl font-bold text-gray-900">Other Jobs in {formattedState}</h2>
